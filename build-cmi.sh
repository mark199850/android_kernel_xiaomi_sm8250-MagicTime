...
# Основной каталог
MAINPATH=/home/mark # измените, если необходимо

# Каталог ядра
KERNEL_DIR=$MAINPATH/kernel
KERNEL_PATH=$KERNEL_DIR/android_kernel_xiaomi_sm8250-MagicTime
...
check_and_wget() {
    ...
    if [ ! -d "$dir" ]; then
        ...
        cd $dir
        wget $repo
        tar -zxvf Clang-20.0.0git-20241013.tar.gz
        rm -rf Clang-20.0.0git-20241013.tar.gz
        cd ../android_kernel_xiaomi_sm8250-MagicTime
    fi
}

# Клонирование инструментов компиляции, если они не существуют
check_and_wget $CLANG_DIR https://github.com/ZyCromerZ/Clang/releases/download/20.0.0git-20241013-release/Clang-20.0.0git-20241013.tar.gz
